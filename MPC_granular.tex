...
\item the lower fidelity 2-DOF model inputs more command changes while negotiating obstacle course 1;
\todo[inline]{Is this really true?  I'm not sure I get this from Fig.~\ref{fig:SteeringCommandsField1}}
...
\item the resulting trajectories are closer to the obstacles when navigating on granular terrain
\todo[inline]{Is this really true?!?!  Doesn't seem so from the figures!}
\item the required controller effort is lower when navigating on granular terrain;
\todo[inline]{This seems surprising.  Deserves an explanation!}
\item the maximum lateral accelerations are similar on both types of terrain
\todo[inline]{Is this really true?!?! Doesn't seem so from the tables!}
\end{itemize}
...
Similar conclusions can be reached comparing the performance of the 2-DOF model when used to control a vehicle on either rigid or granular terrain (comparison of tests 1 and 3). On rigid ground, the 2-DOF controller does a better job of approaching the obstacles more closely. 
\todo[inline]{But that is different from what was said for the 14-DOF model.  How are then the conclusion 'similar'?!?!}
The controller effort on rigid terrain (test 1) is lower than that required on granular terrain (test 3).
\todo[inline]{Again, this is different than what we see for the 14-DOF model!}
